    ...
    try {
        ...
        switch (event.type) {
            case "payment_intent.succeeded": {
                ...
                // ==========================================
                console.log(`📦 Sending order/placed event for ${orderId}`);
                await inngest.send({
                    name: "order/placed",
                    data: {
                        orderId,
                ...
                for (const item of orderItems) {
                    ...
                    console.log(`📦 Stock update event sent for product ${item.product}`);
                }
                console.log(`🚚 Delivery process started for order ${orderId}`);
                break;
            }
            // ==========================================
            // PAYMENT FAILED
            ...
            case "payment_intent.canceled": {
                ...
                console.log(`❌ Payment failed/canceled: ${failureOrderId}`);
                if (failureOrderId) {
                    try {
                        ...
                        console.log(`🗑️ Order ${failureOrderId} deleted`);
                    }
                    catch (err) {
                        if (err.code === "P2025") {
